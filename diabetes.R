...
# Loading the data
raw.data <- read.csv("/home/sunbeam/R_loveeshbhat_imarticus/programs/R_project/github/provided_files
                     /diabetes_allcols.csv",sep = ",", header = T, na.strings = "?")
head(raw.data)
summary(raw.data)

## For getting the readmission prediction we dont need many columns hence we should be selecting only relevant 
## values to make it simpler.Also we need to remove those columns which mainly has NA values.
...
## According to PCA, The columns that are of utmost importance are as follows: age discharged_to 
## time_in_hospital num_lab_procedures num_procedures num_medications number_outpatient number_emergency 
## number_inpatient number_diagnoses insulin change diabetesMed diag1 diag2 diag3 A1Cresult
...
## Regression - Since the output variable readmitted is a categorical variable we need to find logistic 
## regression instead of linear Regression First by taking all the dimensions, in order to get the 
## significance.
